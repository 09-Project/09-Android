...
class MemberCompletedViewModel(private val repository: Repository) : ViewModel() {
    val memberCompletedResponse : MutableLiveData<Response<List<PostValue>>> = MutableLiveData()

    fun memberCompleted(memberId: String) {
        viewModelScope.launch {
            val response = repository.memberCompleted(memberId)
            memberCompletedResponse.value = response
        }
    }
}